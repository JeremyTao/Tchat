 ...
 *  buttom
 *
 *  @param hiqModel hiqModel description
 *  @param hiqUrl   hiqUrl description
 *  @param loqUrl   loqUrl description
 */
- (void)upsauto_setBackgroundImageWithURL:(BOOL)hiqModel hiqUrl:(NSURL *)hiqUrl loqUrl:(NSURL *)loqUrl {
    ...
    SDWebImageCompletionBlock completedBlock = nil;
    
    if (hiqUrl) {
        __weak UIButton *wself = self;
        id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadImageWithURLExtend:hiqModel hiqUrl:hiqUrl lowqUrl:loqUrl options:options completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished) {
            ...
            dispatch_main_sync_safe(^{
                ...
                if (completedBlock && finished) {
                    completedBlock(image, error, cacheType, hiqUrl);
                }
            });
        }];
        ...
        dispatch_main_async_safe(^{
            ...
            if (completedBlock) {
                completedBlock(nil, error, SDImageCacheTypeNone, hiqUrl);
            }
        });
    }
}
