...
- (void)imageDidClicked:(UITapGestureRecognizer *)sender
{
    ...
    if (!self.isBig) { // 放大
            ...
            NSNotification *notification = [[NSNotification alloc] initWithName:PYBigImageDidClikedNotification object:nil userInfo:userInfo];
...
- (void)setPhoto:(PYPhoto *)photo
{
    ...
    UIImage *placeholdeerImage = nil;
    if (self.isBig) { // 图片浏览（放大）, 原图具有优先级
        // 获取缩略图
        placeholdeerImage = self.photo.thumbnailImage ? self.photo.thumbnailImage : nil;
