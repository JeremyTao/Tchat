...
#import "IBOneCollectionViewCell.h"
...
+ (instancetype)cycleScrollViewWithFrame:(CGRect)frame delegate:(id<SDCycleScrollViewDelegate>)delegate placeholderImage:(UIImage *)placeholderImage and:(BOOL)isClose
{
    SDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
    cycleScrollView.delegate = delegate;
    cycleScrollView.placeholderImage = placeholderImage;
    cycleScrollView.closeLookBigImg = isClose;
...
- (void)setupMainView
{
    ...
    if (_closeLookBigImg) {
        [mainView registerClass:[SDCollectionViewCell class] forCellWithReuseIdentifier:@"SDCollectionViewCell"];
    }else{
        [mainView registerClass:[IBOneCollectionViewCell class] forCellWithReuseIdentifier:@"IBOneCollectionViewCell"];
...
- (void)setCustomPageControlDotImage:(UIImage *)image isCurrentPageDot:(BOOL)isCurrentPageDot
{
    ...
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        TAPageControl *pageControl = (TAPageControl *)_pageControl;
        pageControl.isCustomizedDotImage = _isCustomizedDotImage;
        if (isCurrentPageDot) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!_closeLookBigImg) {
        IBOneCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"IBOneCollectionViewCell" forIndexPath:indexPath];
        long itemIndex = indexPath.item % self.imagePathsGroup.count;
        
        NSString *imagePath = self.imagePathsGroup[itemIndex];
        
        if (_imagePathsGroup.count > 0) {
            [cell confi:_imagePathsGroup andIndex:itemIndex];
        }
        if ([imagePath isKindOfClass:[NSString class]]) {
            if ([imagePath hasPrefix:@"/img"] || [imagePath hasPrefix:@"alioss_"]) {
                [cell.imageView setImageUPSUrl:imagePath];
