 ...
 */
static NSInteger const kDefaultSpacingBetweenDots = 15;

/**
 *  Default dot size
 */
static CGSize const kDefaultDotSize = {16, 16};
...
- (void)updateDotFrame:(UIView *)dot atIndex:(NSInteger)index
{
    ...
    if (_isCustomizedDotImage) {
        
        dot.frame = CGRectMake(x , y, 16, 16);
        dot.layer.cornerRadius = 8;
        dot.layer.masksToBounds = YES;
        self.pagaNumberLabel.text = [NSString stringWithFormat:@"%ld",index + 1];
        self.pagaNumberLabel.textColor = [UIColor blackColor];
        self.pagaNumberLabel.backgroundColor = [UIColor whiteColor];
        
        
        UIImageView *tempView = _dots[0];
        
        for (UILabel *temp in tempView.subviews) {
            if (_isCustomizedDotImage) {
                temp.textColor = [UIColor whiteColor];
                temp.backgroundColor = commonBlueColor;
            }
        }
    } else {
...
//        NSLog(@"%@", NSStringFromCGRect(dotView.frame));
    ...
    if (dotView) {
        if (_isCustomizedDotImage) {
            self.pagaNumberLabel        = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 16, 16)];
            self.pagaNumberLabel.backgroundColor = [UIColor whiteColor];
            self.pagaNumberLabel.layer.cornerRadius = 8;
            self.pagaNumberLabel.layer.masksToBounds = YES;
            self.pagaNumberLabel.font = [UIFont systemFontOfSize:14.0f];
            self.pagaNumberLabel.textAlignment = NSTextAlignmentCenter;
            [dotView addSubview:self.pagaNumberLabel];
        }
...
- (void)setCurrentPage:(NSInteger)currentPage
{
    if (_dots.count > 0) {
        ...
        UIImageView *tempView = _dots[currentPage];
