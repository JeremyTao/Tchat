...
- (void)showPhotosToWindow:(PYPhotoBrowseView *)window
{
    ...
    if (copyView.frame.size.height == 0) {
        return;
    }
    // 转移坐标系
...
- (void)hiddenPhoto
{
    ...
    // 计算原始窗口的frame
    // 转移坐标系
     CGRect beginFrame = CGRectMake(0, 0, self.selectedPhotoView.orignalFrame.size.width, self.selectedPhotoView.orignalFrame.size.height);
...
//    [self.beginView.photosView setPhotos:self.selectedPhotoView.photosView.photos];
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        self.beginView.center = self.selectedPhotoView.myPoint;
    } completion:^(BOOL finished) {
        self.scaling = NO;
