...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView {
    if (_redBoarder) {
        for (UIView *subView in self.subViewArr) {
            subView.borderColor = RedPacketColor;
        }
    }
    return YES;
}
- (BOOL)textViewShouldEndEditing:(UITextView *)textView {
    if (_redBoarder) {
        for (UIView *subView in self.subViewArr) {
            subView.borderColor = RGB_COLOR_HEX(0xB3B3B3);
        }
    }
    return YES;
}
...
-(void)mq_verCodeViewWithMaxLenght{
    ...
    for (int i=0; i<self.maxLenght; i++) {
        ...
        [subView mas_makeConstraints:^(MASConstraintMaker *make) {
            if (lastView) {
                make.left.equalTo(lastView.mas_right).with.offset(padding + 8);
            }else{
                make.left.equalTo(@(0));
            }
        ...
        [subLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.centerY.equalTo(subView).with.offset(-1);
        }];
        lastView = subView;
...
-(NSMutableArray *)subViewArr{
    if (!_subViewArr) {
        _subViewArr = [NSMutableArray new];
    }
    return _subViewArr;
}
