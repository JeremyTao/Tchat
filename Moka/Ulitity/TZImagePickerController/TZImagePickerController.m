...
//    }
    ...
    textAttrs[NSForegroundColorAttributeName] = commonBlueColor;
...
- (void)pushToPhotoPickerVc {
    ...
    if (_pushToPhotoPickerVc) {
        ...
        [[TZImageManager manager] getCameraRollAlbum:self.allowPickingVideo allowPickingImage:self.allowPickingImage completion:^(TZAlbumModel *model) {
            ...
            [strongSelf pushViewController:photoPickerVc animated:YES];
...
//    self.navigationItem.title = @"照片";
    ...
    self.title = @"照片";
    [self setNowCommeNaviGationBar:self.title andRightbtnImage:@"取消" andTarget:self SEL:@selector(cancel)];
    [self configTableView];
}
-(void)setNowCommeNaviGationBar:(NSString *)title andRightbtnImage:(NSString *)rightName andTarget:(id)target SEL:(SEL)method {
    ...
    titleLable.textColor = [UIColor colorWithWhite:0.35 alpha:1.000];
    ...
    UIButton *rightBtn = [MyControlUIParts createButtonWithFrame:(CGRectMake(0, 0, 60, 44)) target:target SEL:method title:nil andBG:nil andRadius:0 andTitleColor:[UIColor whiteColor] andtitleFont:[UIFont systemFontOfSize:17.0]];
...
- (void)configTableView {
    TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
    [[TZImageManager manager] getAllAlbums:NO allowPickingImage:imagePickerVc.allowPickingImage completion:^(NSArray<TZAlbumModel *> *models) {
        _albumArr = [NSMutableArray arrayWithArray:models];
        for (TZAlbumModel *albumModel in _albumArr) {
            albumModel.selectedModels = imagePickerVc.selectedModels;
        }
//        [_albumArr removeFirstObject];
        if (!_tableView) {
            CGFloat top = 44;
            if (iOS7Later) top += 20;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
    cell.selectedCountButton.backgroundColor = commonBlueColor;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [photoPickerVc setBackButtonClickHandle:^(TZAlbumModel *model) {
        STRONG_SELF;
        [strongSelf.albumArr replaceObjectAtIndex:indexPath.row withObject:model];
    }];
