...
- (void)viewDidLoad {
    ...
    [self.pageViewController.view addGestureRecognizer:self.longPressGestureRecognizer];
...
- (void)commonInitWithPhotos:(NSArray *)photos initialPhoto:(id <NYTPhoto>)initialPhoto delegate:(id<NYTPhotosViewControllerDelegate>)delegate {
    ...
    _longPressGestureRecognizer = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(didlongPressWithGestureRecognizer:)];
...
- (void)didlongPressWithGestureRecognizer:(UILongPressGestureRecognizer *)longPressGestureRecognizer {
...
- (void)photoViewController:(NYTPhotoViewController *)photoViewController didLongPressWithGestureRecognizer:(UILongPressGestureRecognizer *)longPressGestureRecognizer {
    ...
    BOOL clientDidHandle = NO;
    //弹出保存图片
    if (longPressGestureRecognizer.state == UIGestureRecognizerStateBegan) {  // 长按结束
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0) {
        NSLog(@"保存到相册");
        UIImageWriteToSavedPhotosAlbum(self.currentlyDisplayedPhoto.image, self, @selector(image: didFinishSavingWithError: contextInfo:), nil);
