...
- (void)setup
{
    ...
    UILabel *titleLabel = [self setupTitleLabel:nil];
...
- (NSArray *)addAndLayoutTagsWithTagsContentView:(UIView *)contentView tagTexts:(NSArray<NSString *> *)tagTexts;
{
    ...
    for (int i = 0; i < tagTexts.count; i++) {
        ...
        [label addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tagLabelDidClick:)]];
        label.tag = i;
//        UIView *baseView = [[UIView alloc] initWithFrame:label.bounds];
...
- (void)setHotSearchStyle:(PYHotSearchStyle)hotSearchStyle
{
    ...
    switch (hotSearchStyle) {
        ...
        case PYHotSearchStyleARCBorderTag:
            for (UILabel *tag in self.hotSearchTags) {
                tag.backgroundColor = [UIColor clearColor];
                tag.layer.borderColor = RGB_COLOR_HEX(0xA58DFF).CGColor;
...
- (void)tagDidCLick:(UITapGestureRecognizer *)gr
{
    ...
    PYSEARCH_LOG(@"选择标签 %@", label.text);
    
    
    
}

- (void)tagLabelDidClick:(UITapGestureRecognizer *)gr {
    UILabel *label = (UILabel *)gr.view;
    NSInteger index = label.tag;
    PYSEARCH_LOG(@"点击标签 %@, index %ld", label.text, (long)index);
    
    //从模型数组取出model
    MKInterestTagModel *tagModel = _tagModelsArray[index];
...
- (void)drawGradientLayerOnView:(UIView *)view {
    CAGradientLayer *gradientLayer = [[CAGradientLayer alloc] init];
    gradientLayer.cornerRadius = view.height * 0.5;
